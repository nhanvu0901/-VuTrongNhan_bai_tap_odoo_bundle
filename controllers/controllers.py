            ...
            for bundle in combo_list:
                ...
                if (bundle.discount_rule == "discount_total" and bundle.bundle_type == "bundle"):
                    ...
                    combo_info_total = {
                        ...
                        "start_time": bundle.start_time.strftime("%m/%d/%Y") if bundle.indefinite_bundle == False else '',
    ...
    def compare_combo(self,request):
        ...
        if request.jsonrequest:
            ...
            for combo in discount_combo:
                if (len(combo.product_total) == len(item_list) or len(combo.product_each) == len(item_list) or len(combo.product_tier) == len(item_list)):
                    count = 0
                    for item in item_list:
                        combo_exist = request.env['bundle.product.line'].sudo().search(
            ...
            if bundle:
                ...
                if (bundle.discount_rule == "discount_total" and bundle.bundle_type == "bundle"):
                    ...
                    for item in price_list:
                        ...
                        if (bundle.discount_type == 'per'):
                            sum = total_money - total_money *(int(bundle.discount_value)/100)

                        else:
                            sum = total_money - int(bundle.discount_value)
                    ...
                    for item in price_list:
                        total_money = int(item.get("product_price")) * int(item.get("quantity"))
                        if (bundle.discount_type == 'per'):
                            sum += total_money-total_money *(int(item.get("product_discount"))/100)

                        else:
                            sum += total_money- int(item.get("product_discount"))
